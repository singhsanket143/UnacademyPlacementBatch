...
void helper(string ip, int idx, string output, int count) {
	...
	// self work -> we will just try to find the first valid integer
	for(int i = 1; i < 4; i++) {
		if(idx + i > ip.size()) break;
		string nextPossibleNum = ip.substr(idx, i);
		if((nextPossibleNum.size() > 1 and nextPossibleNum[0] == '0')) break;
		if(i == 3 and stoi(nextPossibleNum) >= 256) break;
